...
//            Toast.makeText(ctx,"₹ "+location.price,Toast.LENGTH_LONG).show()
            ...
            holder.cardRowMain.setOnClickListener {
                if(key==""){
                mDatabase.child("users").child(userId).child("cart").child("areatours").child(location.id).setValue(location).addOnSuccessListener {
                    Glide.with(ctx).load(R.drawable.ic_check).into(holder.check)
                    key="0"
                }}else{
                    mDatabase.child("users").child(userId).child("cart").child("areatours").child(location.id).removeValue().addOnSuccessListener {
